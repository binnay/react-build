...
var NeedsPropagation =
  /*             */
  131072; // Static tags describe aspects of a fiber that are not specific to a render,
...
var MountPassiveDev =
  /*              */
  1048576; // Groups of flags that are used in the commit phase to skip over trees that
...
function formatLanes$1(laneOrLanes) {
...
function markCommitStarted(lanes) {
  if (enableSchedulingProfiler) {
    if (supportsUserTimingV3) {
      markAndClear("--commit-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markLayoutEffectsStarted(lanes) {
  if (enableSchedulingProfiler) {
    if (supportsUserTimingV3) {
      markAndClear("--layout-effects-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markPassiveEffectsStarted(lanes) {
  if (enableSchedulingProfiler) {
    if (supportsUserTimingV3) {
      markAndClear("--passive-effects-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markRenderStarted(lanes) {
  if (enableSchedulingProfiler) {
    if (supportsUserTimingV3) {
      markAndClear("--render-start-" + formatLanes$1(lanes));
    }
  }
}
...
function markRenderScheduled(lane) {
  if (enableSchedulingProfiler) {
    if (supportsUserTimingV3) {
      markAndClear("--schedule-render-" + formatLanes$1(lane));
    }
  }
}
function markForceUpdateScheduled(fiber, lane) {
  if (enableSchedulingProfiler) {
    if (supportsUserTimingV3) {
      ...
      markAndClear(
        "--schedule-forced-update-" + formatLanes$1(lane) + "-" + componentName
      );
    }
  }
}
function markStateUpdateScheduled(fiber, lane) {
  if (enableSchedulingProfiler) {
    if (supportsUserTimingV3) {
      ...
      markAndClear(
        "--schedule-state-update-" + formatLanes$1(lane) + "-" + componentName
      );
    }
  }
}
    ...
    if (list !== null) {
      ...
      findChangedDep: while (dep !== null) {
        ...
        for (var i = 0; i < contexts.length; i += 2) {
          ...
          var changedBits = contexts[i + 1]; // Check if the context matches.
...
function mountLayoutEffect(create, deps) {
  if (
    ...
    return mountEffectImpl(MountLayoutDev | Update, Layout, create, deps);
  } else {
